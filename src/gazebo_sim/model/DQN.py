...
def build_dqn_models( n_actions, input_dims, fc1_dims, fc2_dims, lr, target_network="no", ckpt=None):
        out_size = n_actions
        ...
        model.add(tf.keras.layers.Dense(out_size, activation=None));
        optimizer=tf.optimizers.Adam(learning_rate=lr) ;
        model.compile(optimizer=optimizer)
        model.summary()
        print(target_network, "WTF!!!") ;
...
def build_dueling_models(n_actions, input_dims, fc1_dims, fc2_dims, lr, train_batch_size):
    optimizer=tf.optimizers.Adam(learning_rate=lr) ;
