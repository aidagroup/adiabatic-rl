...
These kill old processes, start Gazebo headless, and run the experiment with full CLI flags. Pass your root and project directories as args:
```
bash colored-lf.bash /absolute/root/path cvpr2025
# or
bash mono-lf.bash   /absolute/root/path cvpr2025
bash po-4-task.bash /absolute/root/path cvpr2025
```
...
sys.argv=[sys.argv[0],
  ...
  '--exp_id','quick_colored','--root_dir','REPLACE_WITH_ABS_PROJECT_PATH']
...
## Philosophy
There is a rather loose coupling between an Agent, a Learner and an Environment, each modeled by a separate instance. Envrionment instances follow the OpenAI Gym 
...
## Tasks and babbling
Each CRL exp is partitioned into tasks. These are assumed to be numbered consecuticely, starting at 0. 
...
## Bash file
Bash files for starting an experiment should be self-contained, export alle necessary environment variables and contain the invocation of the experiment main file with all parameters.
The invocation should be enclosed in # +++ and # --- to be recogized by the slurm-exp tool.
...
variables will have to be set prior to this call in the bash file. All of the sections are annotated with comments to you should have an easy time
doing this. The existing bash files allow a root directory and preoject subdirectory to be set from the command line as ${1} and ${2}. 
The absolute path to the project is then ${1}/${2}. If no parameters are specified, entries from the bas files itself are used. Useful if you want
...
If you can create an SSH tunnel, pictures can be seen in real-time in your browser by opening image.html, which reloads certain image files peiodically.
